...
#include "pixmaps/FillSolidsOn.pm"
#include "pixmaps/FillSolidsOff.pm"

#include "bitmaps/q_mark.xbm"

extern Widget  color_pb1, color_pb2, color_pb3, color_pb4,
               color_pb5, color_pb6, color_pb7, color_pb8,
               color_pb9, color_pb10, color_pb11, color_pb12,
               color_pb13, color_pb14, color_pb15, color_pb16;
...
Pixmap fillSolidsOnPix, fillSolidsOffPix;
        ...
        unsigned long bg )
{
  ...
  xpm_ReadAttribs.valuemask = XpmColorSymbols;
  xpm_ReadAttribs.colorsymbols = override_sym;
  xpm_ReadAttribs.numsymbols = 2;
  ...
  status = XpmCreatePixmapFromData(dpy, root, FillSolidsOn, &fillSolidsOnPix, &mask,
                                &xpm_ReadAttribs);
  if (status != XpmSuccess) Abort(GETSTR(10,28, "Cannot initialize button fill solids"));

  status = XpmCreatePixmapFromData(dpy, root, FillSolidsOff, &fillSolidsOffPix, &mask,
                                &xpm_ReadAttribs);
  if (status != XpmSuccess) Abort(GETSTR(10,28, "Cannot initialize button fill solids"));
  ...
  i = 0;
  XtSetArg(args[i], XmNlabelPixmap, fillSolidsOffPix); i++;
...
Init_Widget_List( void )
{
  ...
  StaticWid[COLOR9] = color_pb9;
  StaticWid[COLOR10] = color_pb10;
  StaticWid[COLOR11] = color_pb11;
  StaticWid[COLOR12] = color_pb12;
  StaticWid[COLOR13] = color_pb13;
  StaticWid[COLOR14] = color_pb14;
  StaticWid[COLOR15] = color_pb15;
  StaticWid[COLOR16] = color_pb16;
...
/*** Set TRANSPARENT Button fg/bg colors ***/
  Transparent = xrdb.transparentPixel;
  i = 0;
...
{
  fprintf (stderr, "[%s] %s", progName, str);
  exit(-1);
}
    ...
    for (i=1; i<num; i++)
    {
      ...
      if (cmd_arg[0] == '-')          /* process command line switches */
      {
	   ...
	   if(param_flag[AUTO_FILE]) {
	   		fprintf(stderr, "Ignoring redundant argument: %s\n", cmd[i]);
	   }else {
	       strncpy(start_file, cmd[i], 255);
        ...
        Boolean flag )
{
  Arg args[10];
  int i;
  Display *dpy = XtDisplay(rectangleButton);
  ...
  if (flag == HOLLOW) {
    ...
    XtSetValues(ellipseButton, args, i);
    i = 0;
    XtSetArg(args[i], XmNlabelPixmap, fillSolidsOnPix); i++;
    XtSetValues(fillToggle, args, i);
   }
   
   XClearArea(dpy, XtWindow(rectangleButton), 0, 0, 0, 0, True);
   XClearArea(dpy, XtWindow(circleButton), 0, 0, 0, 0, True);
   XClearArea(dpy, XtWindow(polygonButton), 0, 0, 0, 0, True);
   XClearArea(dpy, XtWindow(ellipseButton), 0, 0, 0, 0, True);
   XClearArea(dpy, XtWindow(fillToggle), 0, 0, 0, 0, True);
}
